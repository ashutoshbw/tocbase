...
export function tocPleaseCore(headings, config = {}, firstTime = true, nums = []) {
  ...
  for (let i = 0; i < headings.length; i++) {
    ...
    const getDepthNumSpan = className => config.number ? `<span${className ? ` class="${className}"` : ''}>${nums.map(n => n.toLocaleString(config.numberLocale ? config.numberLocale : "en-US")).join(config.hasOwnProperty("numberSeparator") ? config.numberSeparator : '.')}</span>${config.spaceAfterNum ? ' ' : ''}` : '';
